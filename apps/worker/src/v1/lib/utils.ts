...
export async function downloadAndUnzipFile(
  url: string,
  id: string,
  ...
  try {
    ...
    response.data.on("data", (chunk: Buffer) => {
      ...
      if (contentLength && logsManager) {
        const progressPercent = Math.floor(
          (downloadedBytes / parseInt(contentLength)) * 100,
        );
        if (progressPercent <= 100) {
          lastLoggedProgress = progressPercent;
          logsManager
            .addLog(`Download progress: ${progressPercent}%`)
            .catch(() => {});
        }
      }
    });
    ...
    if (logsManager && totalEntries > 0) {
      await logsManager.addLog(
        `   Found ${totalEntries} files/directories in archive`,
      );
    }
    ...
    const topLevel = await fs.promises.readdir(extractedDir, {
      withFileTypes: true,
    });
    ...
    const topFiles = topLevel.filter(
      (e) => e.isFile() && !e.name.startsWith("."),
    );

    let projectPath: string;
    if (
      significantDirs.length === 1 &&
      topFiles.length === 0 &&
      significantDirs[0]
    ) {
      ...
      if (logsManager) {
        await logsManager.addLog(
          ` Project identified: ${significantDirs[0].name}`,
        );
      }
    } else {
      ...
      if (logsManager) {
        await logsManager.addLog(
          " Project identified: (files at archive root)",
        );
      }
    }
    ...
    if (logsManager) {
      await logsManager.addLog(
        `Error during download/extraction: ${errorMessage}`,
        "error",
      );
    }
    throw err;
